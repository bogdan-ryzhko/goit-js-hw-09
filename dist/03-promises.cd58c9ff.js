...
function getArrayPromises(callback, maxAmountPromise) {
    const arrayPromises = [];
    for(let i = 1; i <= maxAmountPromise; i += 1)arrayPromises.push(callback);
    return arrayPromises;
}
function showPromises(array, position, step, firstDelay) {
    array.forEach((promise)=>{
        setTimeout(()=>{
            promise(position, step).then((success)=>(0, _notiflix.Notify).success(success)).catch((error)=>(0, _notiflix.Notify).failure(error));
...
function createPromise(position, delay) {
    ...
    return new Promise((resolve, reject)=>{
        setTimeout(()=>{
            if (shouldResolve) resolve(`Fullfilled promise ${position} in ${delay}`);
